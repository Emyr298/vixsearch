...
use crate::{collection, index, orchestrator::Orchestrator};

struct OrchestratorImpl {
    collection_manager: Arc<dyn collection::Manager>,

    id_locks: DashMap<String, Arc<RwLock<()>>>,
}

impl OrchestratorImpl {
    pub fn new(collection_manager: Arc<dyn collection::Manager>) -> Arc<dyn Orchestrator> {
        Arc::new(OrchestratorImpl {
            collection_manager,
